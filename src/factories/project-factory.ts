...
export default class ProjectFactory {
    public static fromJSON(jsonProject: JsonProject): Project {
        ...
        project.components = jsonProject.components.map(jsonComponent => new ComponentWrapper(ComponentFactory.fromJSON.call(project, jsonComponent)));
        // Add graphs
    ...
    public static toJSON(project: Project): JsonProject {
        const jsonProject = {};
        jsonProject['graphs'] = project.graphs.map(graph => GraphFactory.toJSON(graph));
        jsonProject['cloudProviders'] = project.cloudProviders.map(provider => CloudProviderFactory.toJSON(provider));
        jsonProject['components'] = project.components.map(cw => ComponentFactory.toJSON(cw.component));
        jsonProject['lastId'] = project.lastId;
